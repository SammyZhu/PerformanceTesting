...
myPicList = os.listdir("./pic")  # 读取文件夹中所有的文件名放在数组里
threads = []
...
class MyThread (threading.Thread):
    ...
    def run(self):
            ...
            m = MultipartEncoder(fields={'doc': (myPicList[self.threadID], open('./pic/' + myPicList[self.threadID], 'rb'), 'image/jpeg')})
            ...
            print("第 %d 次请求：\n %s" % (self.threadID+1, r.status_code))


if __name__ == "__main__":
    for i in range(len(myPicList)):
        thread = MyThread(i, "Thread %d" % i, i)  # 创建线程
        thread.start()  # 开启线程
